// ─────────────────────────────────────────────────────────────────────────────
// LOCAL FORK of imgui/backends/imgui_impl_dx9.cpp
//
// Based on upstream ocornut/imgui commit ea6d21687 (the commit the src/imgui
// submodule points at).  We cannot push fixes to the upstream submodule, so
// this patched copy lives in the repo and is compiled INSTEAD of
// imgui\backends\imgui_impl_dx9.cpp -- see build.bat and
// .github/workflows/main.yml.  Do not add the submodule's copy back to either
// build or the linker will report duplicate symbols.
//
// Divergence from upstream, all inside ImGui_ImplDX9_SetupRenderState() except
// where noted.  These make the overlay render correctly on top of a real game
// that leaves arbitrary device state bound:
//
//   * D3DRS_SEPARATEALPHABLENDENABLE forced FALSE, and the SRCBLENDALPHA /
//     DESTBLENDALPHA writes dropped, so the overlay never blends into the
//     backbuffer's alpha channel.
//   * D3DRS_COLORWRITEENABLE pinned to RED|GREEN|BLUE, leaving backbuffer
//     alpha untouched -- captured frames stay opaque.
//   * D3DRS_SRGBWRITEENABLE and D3DSAMP_SRGBTEXTURE forced off so the game's
//     sRGB settings cannot wash out the overlay's colors.
//   * D3DRS_DITHERENABLE, D3DRS_CLIPPLANEENABLE, D3DRS_MULTISAMPLEANTIALIAS,
//     D3DRS_ANTIALIASEDLINEENABLE, D3DRS_VERTEXBLEND and
//     D3DRS_INDEXEDVERTEXBLENDENABLE reset, so leftover game state cannot clip
//     or distort the overlay geometry.
//   * D3DSAMP_MIPFILTER set to D3DTEXF_NONE for the font atlas.
//   * In ImGui_ImplDX9_RenderDrawData(): SetVertexDeclaration(nullptr) before
//     SetFVF, because a vertex declaration left bound by the game otherwise
//     takes precedence over the FVF and garbles the draw.
//
// When bumping the imgui submodule, re-copy the upstream file and re-apply the
// changes above rather than editing the submodule in place.
// ─────────────────────────────────────────────────────────────────────────────
